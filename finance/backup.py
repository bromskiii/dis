...
def history():
    ...
    # Retrieve transaction details from the 'transactions' table
    rows = db.execute(
        """
    ...
    transactions = []
    for row in rows:
        net_price = row["shares"] * row["price"]  # Calculate the net price
        transactions.append({
            "symbol": row["symbol"],
            "shares": row["shares"],
            "price": usd(row["price"]),
            "net_price": usd(net_price),
...
def quote():
    ...
    if request.method == "POST":
        ...
        symbol = request.form.get("symbol").upper()  # Make sure the symbol is uppercase for consistency

        # Check if a symbol was provided
        if not symbol:
            return apology("must provide stock symbol", 400)

        # Lookup the stock data
        stock = lookup(symbol)

        # If the stock lookup fails (returns None), show an apology
        if not stock:
            return apology("invalid stock symbol", 400)

        # If everything is valid, render the quote page with the stock data
        return render_template("quoted.html", stock=stock)

    else:
        # If the request is GET, simply render the page with the form to input the symbol
